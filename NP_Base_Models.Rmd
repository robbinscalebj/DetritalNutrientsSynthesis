...
det_loc_Ptrim <- det_loc %>% filter(initial_P < 0.25, !is.na(P_mass_norm))

Pi_1 <- gam(data = det_loc_Ptrim, P_mass_norm ~  
                        te(Mass_per_loss, initial_P, m = 2)+
                        s(series_index, Mass_per_loss, bs = "re"), 
                   method = "REML", family = gaussian(link = "identity"),select = TRUE) 

Pi_2 <- gam(data = det_loc_Ptrim, P_mass_norm ~  
                     te(Mass_per_loss, initial_P, m = 2)+
                        s(series_index, Mass_per_loss, bs = "re"), 
                   method = "REML", family = gaussian(link = "inverse"),select = TRUE) 


Pi_3 <- gam(data = det_loc_Ptrim, P_mass_norm ~  
                     te(Mass_per_loss, initial_P, m = 2)+
                        s(series_index, Mass_per_loss, bs = "re"), 
                   method = "REML", family = gaussian(link = "log"),select = TRUE) 

Pi_4 <- gam(data = det_loc_Ptrim, P_mass_norm ~  
                     te(Mass_per_loss, initial_P, m = 2)+
                        s(series_index, Mass_per_loss, bs = "re"), 
                   method = "REML", family = Gamma(link = "identity"),select = TRUE)

Pi_5 <- gam(data = det_loc_Ptrim, P_mass_norm ~  
                     te(Mass_per_loss, initial_P, m = 2)+
                        s(series_index, Mass_per_loss, bs = "re"),
                   method = "REML", family = Gamma(link = "inverse"),select = TRUE)
...
map(Pi_names, ~AIC(.)) #best is 6
map(Pi_names, ~appraise(.)) #all are heteroscedastic...
summary(Pi_6) 

base_loc_mod <- Pi_6  #set best model to easier name to remember for later comparisons
...
base_loc.pred<-predict_gam(type = "link", base_loc_mod, length_out = 5000,  exclude_terms = c("s(series_index)", "s(series_index, Mass_per_loss)"), values = list(series_index = NULL, Remain_Mass_Category = NULL, initial_P = c(0.01,0.02,0.05,0.1,0.15,0.23)))%>%
 ...
 base_loc.p<- ggplot()+
  geom_point(data = det_loc_Ptrim, aes(Mass_per_loss, P_mass_norm), alpha = 0.3)+
    geom_smooth(data = base_loc.pred, se = FALSE, aes(x = Mass_per_loss, y = exp(fit)))+
    geom_abline(slope = -0.01, intercept = 1)+
...
gam_fitted <- fitted_samples(base_loc_mod, exclude = "s(series_index,Mass_per_loss", n = n_draws, newdata = P_mass_grid, seed = 1)%>%
  bind_cols(P_mass_grid_exp)%>%mutate(initial_P = as_factor(initial_P), draw = as_factor(draw))


deriv_samples <- gam_fitted%>%arrange(initial_P)%>%group_by(draw, initial_P)%>%mutate(fit_dif = fitted-lag(fitted))%>%mutate(Mass_per_loss = as_factor(Mass_per_loss))%>%ungroup()%>%
...
baseCN_loc.pred<-predict_gam(type = "link", CN_base2, length_out = 5000,  exclude_terms = c("s(series_index)", "s(series_index, Mass_per_loss)"), values = list(series_index = NULL, Remain_Mass_Category = NULL, initial_CN = c(25,40,50,70,90,110,140)))%>%
 ...
 base_loc_CN.p<- ggplot()+
  geom_point(data = det_loc%>%filter(!is.na(CN_ratio)), aes(Mass_per_loss, CN_ratio), alpha = 0.3)+
    ...
    geom_abline(slope = -0.01, intercept = 1)+
   scale_y_continuous(trans = "log", breaks = c(1,5,20,50,150))+
...
```
```{r C:N derivs}
CN_mass_grid <- expand_grid(Mass_per_loss = seq(0,80, by = 0.2), initial_CN = c(25,40,50,70,90,110,140), series_index = 578)
n_draws <- 10000
CN_mass_grid_exp <- map_dfr(seq_len(n_draws), ~CN_mass_grid)


gam_fitted <- fitted_samples(CN_base2, exclude = "s(series_index,Mass_per_loss", n = n_draws, newdata = CN_mass_grid, seed = 1)%>%
...
deriv_samples <- gam_fitted%>%arrange(initial_CN)%>%group_by(draw, initial_CN)%>%mutate(fit_dif = fitted-lag(fitted))%>%mutate(Mass_per_loss = as_factor(Mass_per_loss))%>%ungroup()%>%
...
CP_base2b <- gam(data = det_loc, CP_ratio ~ 
                        te(Mass_per_loss, initial_CP, m = 2),
                 method = "REML", family = Gamma(link = "log"),select = TRUE) 
...
baseCP_loc.pred<-predict_gam(type = "link", CP_base2, length_out = 5000,  exclude_terms = c("s(series_index)", "s(series_index, Mass_per_loss)"), values = list(series_index = NULL, Remain_Mass_Category = NULL, initial_CP = c(800,1200,1600,2000,3000,5000)))%>%
...
gam_fitted <- fitted_samples(CP_base2, exclude = "s(series_index,Mass_per_loss", n = n_draws, newdata = CP_mass_grid, seed = 1)%>%
  bind_cols(CP_mass_grid_exp)%>%mutate(initial_CP = as_factor(initial_CP), draw = as_factor(draw), fitted = log(fitted))
#log transformed fitted values because *ratios*


deriv_samples <- gam_fitted%>%arrange(initial_CP)%>%group_by(draw, initial_CP)%>%mutate(fit_dif = fitted-lag(fitted))%>%mutate(Mass_per_loss = as_factor(Mass_per_loss))%>%ungroup()%>%
