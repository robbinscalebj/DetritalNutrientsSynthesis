...
DetNut.nest <- det2%>%
  group_by(First_Author, Title, Journal, series_index)%>%
  nest()%>%
  ...
  mutate(Series_Measurements = map_dbl(data, nrow),
         ...
         CN_counts = map_dbl(data, ~sum(!is.na(.$CN_molar))),
         CP_counts = map_dbl(data, ~sum(!is.na(.$CP_molar))),
         NP_counts = map_dbl(data, ~sum(!is.na(.$NP_molar))))
