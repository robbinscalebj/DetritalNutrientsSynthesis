...
series_remove <- det_loc_cnp|>filter(!is.na(CN_ratio)&!is.na(initial_CN)&!is.na(no3_avg))|>distinct(series_index)|>
  mutate(series_index = as.numeric(series_index))|>pull()
series_remove <- series_remove[1:2]
...
#fit gam for plotting predictions
  ...
  gam.i_preds <- gam(data = det_loc_cnp, CN_ratio ~  
                  ti(Mass_per_loss) + ti(initial_CN)+
                  ti(Mass_per_loss, initial_CN)+
...
predict_gam(type = "link", gam.i_preds, length_out = 5000,  exclude_terms = c("s(series_index)", "s(series_index, Mass_per_loss)"), values = list(series_index = NULL,Remain_Mass_Category = NULL, initial_CN = c(140,100,70,50,40,25),no3_avg = c(25,50,100)))%>%
...
#ggsave2("C:/Users/caleb/Dropbox/Detrital Nutrients Synth/Manuscript Dev/Temporal Trends ms/Figures/Sensitivity Figures/CN_no3_sens.pdf", CN_no3sens_grid, width = 7.8, height = 9.1, dpi = 600)
...
#ggsave2("C:/Users/caleb/Dropbox/Detrital Nutrients Synth/Manuscript Dev/Temporal Trends ms/Figures/Sensitivity Figures/CN_po4_sens.pdf", CN_srpsens_grid, width = 7.8, height = 9.1, dpi = 600)
