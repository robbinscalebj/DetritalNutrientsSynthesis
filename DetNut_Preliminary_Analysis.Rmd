...
det <- read_csv("C:\\Users\\Caleb_Robbins\\Dropbox\\Detrital Nutrients Synth\\DetNutSynth_Database_preliminary.csv")
...
knots <- data.frame(x=seq(0,4,length=6)) ## create knots
## set up smoother...
sm <- smoothCon(s(Mass_per_loss,k=6,bs="cr"),data = det3,knots=knots)[[1]]
...
off <- det3$Mass_per_loss*0 + 1       ## offset term to force curve through (0, 1)

## fit spline constrained through (0, .6)...
b <- gam(N_mass_norm ~ X - 1 + offset(off),data = det3, paraPen=list(X=list(S)))
lines(det3$Mass_per_loss,predict(b))
```
...
N.gam <- gam(data = det3, N_prop_initial ~ te(Mass_per_loss, initial_N) + s(series_index, bs = "re"), method = "REML")
...
```{r plot predictions }
gam_pred<-predict_gam(N.gam,  exclude_terms = "s(series_index)",values = list(initial_N = c(0.3,0.4,0.5, 0.6,0.8, 1,2,3.5,4.5)))%>%
  mutate(initial_N = as_factor(initial_N))


  ggplot()+
  geom_point(data = gam_pred, aes(x = Mass_per_loss, y = fit, color = initial_N))+
  geom_point(data = det3, aes(Mass_per_loss, N_prop_initial), alpha = 0.05)+
