...
#base model - global change in N mass curves
N.loc <- gam(data = det_loc, N_mass_norm ~ s(Mass_per_loss, k = 15) + s(series_index, Mass_per_loss, bs = "re"), method = "REML", family = Gamma(link = "inverse"))

N.loc <- gam(data = det_loc, N_mass_norm ~ s(Mass_per_loss, k = 15), method = "REML", family = Gamma(link = "inverse"))
summary(N.loc)
appraise(N.loc)
draw(N.loc)

gam_pred<-predict_gam(N.loc, exclude_terms = "s(series_index,Mass_per_loss)")
gam_pred<-predict_gam(N.loc)


  ggplot()+
  geom_point(data = det_loc, aes(Mass_per_loss, N_mass_norm), alpha = 0.3)+
    geom_point(data = gam_pred, aes(x = Mass_per_loss, y = 1/fit))+
    geom_abline(slope = -0.01, intercept = 1)+
    geom_ribbon(data = gam_pred, aes(x = Mass_per_loss, ymin = 1/(fit -se.fit), ymax = 1/(fit +se.fit)), alpha = 0.3)+
    NULL

#simple mass loss model - simpler model to see how well Mass loss predicts (i.e., proportional declines in N mass from initial quantity)
N.loc1 <- gam(data = det_loc, N_mass_norm ~ s(Mass_per_loss, k = 15), method = "REML", family = Gamma(link = "inverse"))

#simple mass loss model + random slope
N.loc.2 <- gam(data = det_loc, N_mass_norm ~ s(Mass_per_loss, k = 15) + s(series_index, Mass_per_loss, bs = "re"), method = "REML", family = Gamma(link = "inverse"))

N.loc.3 <- gam(data = det_loc, N_mass_norm ~ s(Mass_per_loss) + ti(Mass_per_loss, initial_N) + s(series_index, Mass_per_loss, bs = "re") + s(series_index, bs = "re"), method = "REML", family = Gamma(link = "inverse"))

#base model + random intercept - in some ways this doesn't make sense because intercept should be 1
N.loc.ri <- gam(data = det_loc, N_mass_norm ~ s(Mass_per_loss, k = 15) + s(initial_N) + ti(Mass_per_loss, initial_N, k = 16) + s(series_index, bs = "re"), method = "REML", family = Gamma(link = "inverse"))

#base model + random slope - allows sereies-to-series variation in relationship between predictor and N mass, captures autocorr, probably most appropriate model
N.loc.rs <- gam(data = det_loc, N_mass_norm ~ s(Mass_per_loss) + s(initial_N) + ti(Mass_per_loss, initial_N, k = 16) + s(series_index, Mass_per_loss, bs = "re"), method = "REML", family = Gamma(link = "inverse"))

# base model + random slope&intercept - random intercept still may be inappropriate given the data structure
N.loc.rsi <- gam(data = det_loc, N_mass_norm ~ s(Mass_per_loss) + s(initial_N) + ti(Mass_per_loss, initial_N) + s(series_index, bs = "re") +s(series_index, Mass_per_loss, bs = "re"), method = "REML", family = Gamma(link = "inverse"))
...
AIC(N.loc, N.loc.ri,N.loc.rs,N.loc.rsi)

gam_pred<-predict_gam(N.loc.rs, exclude_terms = "s(series_index,Mass_per_loss)",  values = list(initial_N = c(0.4,0.6,0.8, 1,1.5,2.0,2.5)))%>% mutate(initial_N = as_factor(initial_N))


  ggplot()+
  geom_point(data = det_loc, aes(Mass_per_loss, N_mass_norm), alpha = 0.3)+
    geom_point(data = gam_pred, aes(x = Mass_per_loss, y = 1/fit, color = initial_N))+
    geom_abline(slope = -0.01, intercept = 1)+
    NULL
  
  gam_pred1<-predict_gam(N.loc.rsi, exclude_terms = c("s(series_index)", "s(series_index,Mass_per_loss)"),  values = list(initial_N = c(0.4, 0.6,0.8, 1,1.5,2.0,2.5)))%>% mutate(initial_N = as_factor(initial_N))


  ggplot()+
  geom_point(data = det_loc, aes(Mass_per_loss, N_mass), alpha = 0.3)+
    geom_point(data = gam_pred1, aes(x = Mass_per_loss, y = 1/fit, color = initial_N))+
    geom_abline(slope = -0.01, intercept = 1)+
    NULL
  
    gam_pred2<-predict_gam(N.loc, values = list(initial_N = c(0.4, 0.6,0.8, 1,1.5,2.0,2.5)))%>% mutate(initial_N = as_factor(initial_N))


  ggplot()+
  geom_point(data = det_loc, aes(Mass_per_loss, N_mass), alpha = 0.3)+
    geom_point(data = gam_pred2, aes(x = Mass_per_loss, y = 1/fit))+
    geom_abline(slope = -0.01, intercept = 1)+
    NULL
...
N.ext1 <- gam(data = det_loc, N_mass_norm ~ s(Mass_per_loss) + s(Temperature_C) + s(TN_approx_avg) +s(Velocity_m.s) + s(series_index, Mass_per_loss, bs = "re"), method = "REML", family = Gamma(link="inverse"))
