...
N.gam.gamma2 <- gam(data = det3, N_mass_norm ~ te(Mass_per_loss, initial_N, by = Lotic_Lentic, bs = "cr", k = c(15, 10)) + s(series_index, bs = "re"), method = "REML", family = Gamma(link = "log"))
...
gam_pred2<-predict_gam(N.gam.gamma2, exclude_terms = "s(series_index)",  values = list(initial_N = c(0.4,0.5, 0.6,0.8, 1,2,2.5)))%>% mutate(initial_N = as_factor(initial_N))


  ggplot()+
  geom_point(data = gam_pred2, aes(x = Mass_per_loss, y = exp(fit), color = initial_N))+
  geom_point(data = det3[!is.na(det3$Lotic_Lentic),], aes(Mass_per_loss, N_mass_norm), alpha = 0.05)+
    scale_x_continuous(limits = c(0,95))+
    facet_grid(.~Lotic_Lentic)+
    scale_y_continuous(limits = c(0,4), breaks = c(0,0.5,1,2))+
