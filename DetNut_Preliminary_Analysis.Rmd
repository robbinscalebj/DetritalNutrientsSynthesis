...
det_trim<-det%>%
  ...
  ungroup()%>%
  mutate(N_bin = cut_number(initial_N, n = 5))
...
gam_pred<-predict_gam(N.loc.gam, exclude_terms = "s(series_index)",  values = list(initial_N = c(0.3,0.4,0.5, 0.6,0.8, 1,1.5,2.0)))%>% mutate(initial_N = as_factor(initial_N))
  ...
  ggplot()+
  geom_point(data = gam_pred, aes(x = Mass_per_loss, y = exp(fit), shape = initial_N))+
  geom_point(data = det_loc, aes(Mass_per_loss, N_mass_norm, color = N_bin), alpha = 0.3)+
...
N.lec.gam <- gam(data = det_lec, N_mass_norm ~ te(Mass_per_loss, initial_N,  bs = "cr") + s(series_index, bs = "re"), method = "REML", family = Gamma(link = "log"))
...
det_lof <- det_trim%>%
  filter(mesh_cat == "fine", Lotic_Lentic == "Lotic")
...
gam_pred<-predict_gam(N.lof.gam, exclude_terms = "s(series_index)",  values = list(initial_N = c(0.3,0.4,0.5, 0.6,0.8, 1,1.5,2.0,2.5)))%>% mutate(initial_N = as_factor(initial_N))


  ggplot()+
  geom_point(data = gam_pred, aes(x = Mass_per_loss, y = exp(fit), color = initial_N))+
  geom_point(data = det_lof, aes(Mass_per_loss, N_mass_norm), alpha = 0.2)+
    scale_x_continuous(limits = c(0,95))+
    #facet_grid(.~Lotic_Lentic)+
    scale_y_continuous(limits = c(0,3), breaks = c(0,0.5,1,2))+
    geom_abline(slope = -0.01, intercept = 1)+
...
N.gam.gamma0 <- gam(data = det3, N_mass_norm ~ te(Mass_per_loss, initial_N,  bs = "cr") + s(series_index, bs = "re"), method = "REML", family = Gamma(link = "log"))

N.gam.gamma1 <- gam(data = det3, N_mass_norm ~ te(Mass_per_loss, initial_N, by = Lotic_Lentic,  bs = "cr") + s(series_index, bs = "re"), method = "REML", family = Gamma(link = "log"))

N.gam.gamma2 <- gam(data = det3, N_mass_norm ~ te(Mass_per_loss, initial_N, by = Lotic_Lentic,  bs = "cr", k = c(15, 10))+te(Mass_per_loss, initial_N, by = mesh_cat,  bs = "cr", k = c(15, 10)) + s(series_index, bs = "re"), method = "REML", family = Gamma(link = "log"))

N.gam.gamma3 <- gam(data = det3, N_mass_norm ~ te(Mass_per_loss, initial_N, by = mesh_cat,  bs = "cr", k = c(15, 10)) + s(series_index, bs = "re"), method = "REML", family = Gamma(link = "log"))

N.gam.gamma4 <- gam(data = det3, N_mass_norm ~ te(Mass_per_loss, initial_N, by = Lotic_Lentic,  bs = "cr", k = c(15, 10))+te(Mass_per_loss, initial_N, by = mesh_cat,  bs = "cr", k = c(15, 10)), method = "REML", family = Gamma(link = "log"))
