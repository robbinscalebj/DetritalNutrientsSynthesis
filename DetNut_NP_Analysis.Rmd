...
#N mass models
##Base initial N model


So, we'll create a base model with just mass loss as a predictor (plus accounting for effects of dm vs afdm differences, and random effects to take care of individual series-level correlation) to estimate a global tren
...
```{r N mass vs Temperature - full interaction}
...
```{r N mass vs TN - full interaction}
...
```{r N mass vs TP - full interaction}
#N_Ni_TN2 was best most complex model, no need to fit again, but form copied here as reminder
...
#model d doesn't make sense... no chance to decompose TP out...
...
Do Velocity next, see if it'll fit. Lignin almost certainly won't


############
```{r N mass vs initial Lignin}
N_Li1 <- gam(data = det_loc, N_mass_norm ~  
                        te(Mass_per_loss, initial_Lignin)+
                        t2(Mass_per_loss,Remain_Mass_Category,m = 2, bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = gaussian(link = "identity"),select = TRUE) 

N_Li2 <- gam(data = det_loc, N_mass_norm ~
                te(Mass_per_loss, initial_Lignin)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2,bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = gaussian(link = "inverse"),select = TRUE) 
...
N_Li3 <- gam(data = det_loc, N_mass_norm ~  
                te(Mass_per_loss, initial_Lignin)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2,bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = gaussian(link = "log"),select = TRUE) 

N_Li4 <- gam(data = det_loc, N_mass_norm ~  
                te(Mass_per_loss, initial_Lignin)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2,bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = Gamma(link = "identity"),select = TRUE) 

N_Li5 <- gam(data = det_loc, N_mass_norm ~  
                te(Mass_per_loss, initial_Lignin)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2,bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = Gamma(link = "inverse"),select = TRUE) 

N_Li6 <- gam(data = det_loc, N_mass_norm ~  
                te(Mass_per_loss, initial_Lignin)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2,bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = Gamma(link = "log"),select = TRUE) 

Li_mod_names <- list(N_Li1, N_Li2,N_Li3,N_Li4,N_Li5,N_Li6)
map(Li_mod_names, ~AIC(.)) #1,2,3 probably equivocal AIC, 2 slightly better than 1 or 3, by about 4
map(Li_mod_names, ~appraise(.)) #2 maybe better on resids

summary(N_Li2)#suggests this is not significant predictor



base_Li_mod <- N_Li2 #set best model to easier name to remember for later comparisons


#plot predictions
base_loc_Li.pred<-predict_gam(type = "link", base_Li_mod, length_out = 5000,  exclude_terms = c("s(series_index)", "s(series_index, Mass_per_loss)"), values = list(series_index = NULL,Remain_Mass_Category = NULL, initial_Lignin = c(5,15,25,35)))%>%
   mutate(initial_Lignin = as_factor(initial_Lignin))
  


 base_loc_Li.p<- ggplot()+
  geom_point(data = det_loc%>%filter(!is.na(initial_Lignin)), aes(Mass_per_loss, N_mass_norm), alpha = 0.3)+
    geom_smooth(data = base_loc_Li.pred, se = FALSE, aes(x = Mass_per_loss, y = 1/fit, color = initial_Lignin))+
    geom_abline(slope = -0.01, intercept = 1)+
    #geom_ribbon(data = base_loc.pred, aes(x = Mass_per_loss, ymin = 1/(fit - 2*se.fit), ymax = 1/(fit + 2*se.fit)), alpha = 0.3)+
    NULL
 base_loc_Li.p
```
We can conclude here there's really no discernible effect of initial_Lignin in our dataset. 

```{r N mass vs Temperature}
N_T1 <- gam(data = det_loc, N_mass_norm ~  
                        te(Mass_per_loss, Temperature_C)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2,bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = gaussian(link = "identity"),select = TRUE) 

N_T2 <- gam(data = det_loc, N_mass_norm ~
                te(Mass_per_loss, Temperature_C)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2,bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = gaussian(link = "inverse"),select = TRUE) 
...
N_T3 <- gam(data = det_loc, N_mass_norm ~  
                te(Mass_per_loss, Temperature_C)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2,bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = gaussian(link = "log"),select = TRUE) 

N_T4 <- gam(data = det_loc, N_mass_norm ~  
                te(Mass_per_loss, Temperature_C)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2,bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = Gamma(link = "identity"),select = TRUE) 

N_T5 <- gam(data = det_loc, N_mass_norm ~  
                te(Mass_per_loss, Temperature_C)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2,bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = Gamma(link = "inverse"),select = TRUE) 

N_T6 <- gam(data = det_loc, N_mass_norm ~  
                te(Mass_per_loss, Temperature_C)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2,bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = Gamma(link = "log"),select = TRUE) 

T_mod_names <- list(N_T1, N_T2,N_T3,N_T4,N_T5,N_T6)
map(T_mod_names, ~AIC(.)) #2 clear best, then 3
map(T_mod_names, ~appraise(.)) #2 maybe better on resids, none too great on heteroscedasticity

summary(N_T2)#suggests Temp is not significant predictor



base_T_mod <- N_T2 #set best model to easier name to remember for later comparisons


#plot predictions
base_loc_T.pred<-predict_gam(type = "link", base_T_mod, length_out = 5000,  exclude_terms = c("s(series_index)", "s(series_index, Mass_per_loss)"), values = list(series_index = NULL,Remain_Mass_Category = NULL, Temperature_C = c(5,10,15,20)))%>%
   mutate(Temperature_C = as_factor(Temperature_C))
 ...
 base_loc_T.p<- ggplot()+
  geom_point(data = det_loc%>%filter(!is.na(Temperature_C)), aes(Mass_per_loss, N_mass_norm), alpha = 0.3)+
    geom_smooth(data = base_loc_T.pred, se = FALSE, aes(x = Mass_per_loss, y = 1/fit, color = Temperature_C))+
    geom_abline(slope = -0.01, intercept = 1)+
    #geom_ribbon(data = base_loc.pred, aes(x = Mass_per_loss, ymin = 1/(fit - 2*se.fit), ymax = 1/(fit + 2*se.fit)), alpha = 0.3)+
    NULL
 base_loc_T.p
...
```
No temperature effect supported here.

```{r N mass vs TN}
N_TN1 <- gam(data = det_loc, N_mass_norm ~  
                        te(Mass_per_loss, TN_approx_avg)+
                        t2(Mass_per_loss,Remain_Mass_Category,m = 2, bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = gaussian(link = "identity"),select = TRUE) 

N_TN2 <- gam(data = det_loc, N_mass_norm ~
                te(Mass_per_loss, TN_approx_avg)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2,bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = gaussian(link = "inverse"),select = TRUE) 
...
N_TN3 <- gam(data = det_loc, N_mass_norm ~  
                te(Mass_per_loss, TN_approx_avg)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2,bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = gaussian(link = "log"),select = TRUE) 

N_TN4 <- gam(data = det_loc, N_mass_norm ~  
                te(Mass_per_loss, TN_approx_avg)+
                        t2(Mass_per_loss,Remain_Mass_Category,m = 2, bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = Gamma(link = "identity"),select = TRUE) 

N_TN5 <- gam(data = det_loc, N_mass_norm ~  
                te(Mass_per_loss, TN_approx_avg)+
                        t2(Mass_per_loss,Remain_Mass_Category,m = 2, bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = Gamma(link = "inverse"),select = TRUE) 

N_TN6 <- gam(data = det_loc, N_mass_norm ~  
                te(Mass_per_loss, TN_approx_avg)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2,bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = Gamma(link = "log"),select = TRUE) 

TN_mod_names <- list(N_TN1, N_TN2,N_TN3,N_TN4,N_TN5,N_TN6)
map(TN_mod_names, ~AIC(.)) #3 clear best, then 2,6
map(TN_mod_names, ~appraise(.)) #2 maybe better on resids, none too great on heteroscedasticity

summary(N_TN3)#suggests TN probably  not significant predictor



base_TN_mod <- N_TN3.a #set best model to easier name to remember for later comparisons


#plot predictions
base_loc_TN.pred<-predict_gam(type = "link", base_TN_mod, length_out = 5000,  exclude_terms = c("s(series_index)", "s(series_index, Mass_per_loss)"), values = list(series_index = NULL,Remain_Mass_Category = NULL, TN_approx_avg = c(250,750,1500)))%>%
   mutate(TN_approx_avg = as_factor(TN_approx_avg))
  


 base_loc_TN.p<- ggplot()+
  geom_point(data = det_loc%>%filter(!is.na(TN_approx_avg)), aes(Mass_per_loss, N_mass_norm), alpha = 0.3)+
    geom_smooth(data = base_loc_TN.pred, se = FALSE, aes(x = Mass_per_loss, y = exp(fit), color = TN_approx_avg))+
    geom_abline(slope = -0.01, intercept = 1)+
    #geom_ribbon(data = base_loc.pred, aes(x = Mass_per_loss, ymin = 1/(fit - 2*se.fit), ymax = 1/(fit + 2*se.fit)), alpha = 0.3)+
    NULL
 base_loc_TN.p
...
```{r N mass vs TP}
N_TP1 <- gam(data = det_loc, N_mass_norm ~  
                        te(Mass_per_loss, TP_approx_avg)+
                        t2(Mass_per_loss,Remain_Mass_Category,m = 2, bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = gaussian(link = "identity"),select = TRUE) 

N_TP2 <- gam(data = det_loc, N_mass_norm ~
                te(Mass_per_loss, TP_approx_avg)+
                        t2(Mass_per_loss,Remain_Mass_Category,m = 2, bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = gaussian(link = "inverse"),select = TRUE) 
...
N_TP3 <- gam(data = det_loc, N_mass_norm ~  
                te(Mass_per_loss, TP_approx_avg)+
                        t2(Mass_per_loss,Remain_Mass_Category,m = 2, bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = gaussian(link = "log"),select = TRUE) 
...
N_TP4 <- gam(data = det_loc, N_mass_norm ~  
                te(Mass_per_loss, TP_approx_avg)+
                        t2(Mass_per_loss,Remain_Mass_Category,m = 2, bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = Gamma(link = "identity"),select = TRUE) 

N_TP5 <- gam(data = det_loc, N_mass_norm ~  
                te(Mass_per_loss, TP_approx_avg)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2,bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = Gamma(link = "inverse"),select = TRUE) 

N_TP6 <- gam(data = det_loc, N_mass_norm ~  
                te(Mass_per_loss, TP_approx_avg)+
                        t2(Mass_per_loss,Remain_Mass_Category,m = 2, bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = Gamma(link = "log"),select = TRUE) 

TP_mod_names <- list(N_TP1, N_TP2,N_TP3,N_TP4,N_TP5,N_TP6)
map(TP_mod_names, ~AIC(.)) #3  best, then 2
map(TP_mod_names, ~appraise(.)) #2 maybe better on resids, none too great on heteroscedasticity

summary(N_TP3.a)#suggests TP might be sig. predictor



base_TP_mod <- N_TP3.a #set best model to easier name to remember for later comparisons


#plot predictions
base_loc_TP.pred<-predict_gam(type = "link", base_TP_mod, length_out = 5000,  exclude_terms = c("s(series_index)", "s(series_index, Mass_per_loss)"), values = list(series_index = NULL,Remain_Mass_Category = NULL, TP_approx_avg = c(50,100,200,300,500)))%>%
   mutate(TP_approx_avg = as_factor(TP_approx_avg))
  


 base_loc_TP.p<- ggplot()+
  geom_point(data = det_loc%>%filter(!is.na(TP_approx_avg)), aes(Mass_per_loss, N_mass_norm), alpha = 0.3)+
    geom_smooth(data = base_loc_TP.pred, se = FALSE, aes(x = Mass_per_loss, y = exp(fit), color = TP_approx_avg))+
    geom_abline(slope = -0.01, intercept = 1)+
    #geom_ribbon(data = base_loc.pred, aes(x = Mass_per_loss, ymin = 1/(fit - 2*se.fit), ymax = 1/(fit + 2*se.fit)), alpha = 0.3)+
    NULL
 base_loc_TP.p
...
```{r N mass vs Velocity}
N_vel1 <- gam(data = det_loc, N_mass_norm ~  
                        te(Mass_per_loss, Velocity_m.s)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2, bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = gaussian(link = "identity"),select = TRUE) 

N_vel2 <- gam(data = det_loc, N_mass_norm ~
                te(Mass_per_loss, Velocity_m.s)+
                        t2(Mass_per_loss,Remain_Mass_Category,m = 2, bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = gaussian(link = "inverse"),select = TRUE) 
...
N_vel3 <- gam(data = det_loc, N_mass_norm ~  
                te(Mass_per_loss, Velocity_m.s)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2,bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = gaussian(link = "log"),select = TRUE) 
...
N_vel4 <- gam(data = det_loc, N_mass_norm ~  
                te(Mass_per_loss, Velocity_m.s)+
                        t2(Mass_per_loss,Remain_Mass_Category,m = 2, bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = Gamma(link = "identity"),select = TRUE) 

N_vel5 <- gam(data = det_loc, N_mass_norm ~  
                te(Mass_per_loss, Velocity_m.s)+
                        t2(Mass_per_loss,Remain_Mass_Category,m = 2, bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = Gamma(link = "inverse"),select = TRUE) 

N_vel6 <- gam(data = det_loc, N_mass_norm ~  
                te(Mass_per_loss, Velocity_m.s)+
                        t2(Mass_per_loss,Remain_Mass_Category,m = 2, bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = Gamma(link = "log"),select = TRUE) 

vel_mod_names <- list(N_vel1, N_vel2,N_vel3,N_vel4,N_vel5,N_vel6)
map(vel_mod_names, ~AIC(.)) #3  best, then 2
map(vel_mod_names, ~appraise(.)) #2 maybe better on resids, none too great on heteroscedasticity

summary(N_vel3)#suggests TP might be sig. predictor



base_vel_mod <- N_vel3 #set best model to easier name to remember for later comparisons


#plot predictions
base_loc_vel.pred<-predict_gam(type = "link", base_vel_mod, length_out = 5000,  exclude_terms = c("s(series_index)", "s(series_index, Mass_per_loss)"), values = list(series_index = NULL,Remain_Mass_Category = NULL, Velocity_m.s = c(0.1,0.2,0.4)))%>%
   mutate(Velocity_m.s = as_factor(Velocity_m.s))
  


 base_loc_vel.p<- ggplot()+
  geom_point(data = det_loc%>%filter(!is.na(Velocity_m.s)), aes(Mass_per_loss, N_mass_norm), alpha = 0.3)+
    geom_smooth(data = base_loc_vel.pred, se = FALSE, aes(x = Mass_per_loss, y = exp(fit), color = Velocity_m.s))+
    geom_abline(slope = -0.01, intercept = 1)+
    #geom_ribbon(data = base_loc.pred, aes(x = Mass_per_loss, ymin = 1/(fit - 2*se.fit), ymax = 1/(fit + 2*se.fit)), alpha = 0.3)+
    NULL
 base_loc_vel.p
...
```{r}
Np_base <- gam(data = det_loc, N_per ~  
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2, bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re")+s(series_index, bs = "re"), 
             method = "REML", family = gaussian(link = "log"),select = TRUE) 

Np_base2 <- gam(data = det_loc, N_per ~  
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2, bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = gaussian(link = "log"),select = TRUE) 


AIC(Np_base, Np_base2)
map(list(Np_base,Np_base2), appraise)

base_loc_Np.pred<-predict_gam(type = "link", base_Np_mod, length_out = 5000,  exclude_terms = c("s(series_index)", "s(series_index, Mass_per_loss)"), values = list(series_index = NULL))




 base_loc_Np.p<- ggplot()+
  geom_point(data = det_loc%>%filter(!is.na(N_per)), aes(Mass_per_loss, N_per), alpha = 0.3)+
    geom_smooth(data = base_loc_Np.pred, se = FALSE, aes(x = Mass_per_loss, y = 1/fit))+
    geom_abline(slope = -0.01, intercept = 1)+
    #geom_ribbon(data = base_loc.pred, aes(x = Mass_per_loss, ymin = 1/(fit - 2*se.fit), ymax = 1/(fit + 2*se.fit)), alpha = 0.3)+
    NULL
 base_loc_Np.p
...
```{r N per by initial N}



Nper_Ni1 <- gam(data = det_loc, N_prop_initial ~  
                        te(Mass_per_loss, initial_N, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2, bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = gaussian(link = "identity"),select = TRUE) 

Nper_Ni2 <- gam(data = det_loc, N_prop_initial ~  
                        te(Mass_per_loss, initial_N, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2, bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = gaussian(link = "inverse"),select = TRUE) 

Nper_Ni3 <- gam(data = det_loc, N_prop_initial ~  
                        te(Mass_per_loss, initial_N, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2, bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = gaussian(link = "log"),select = TRUE) 

Nper_Ni4 <- gam(data = det_loc, N_prop_initial ~  
                        te(Mass_per_loss, initial_N, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2, bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = Gamma(link = "identity"),select = TRUE) 

Nper_Ni5 <- gam(data = det_loc, N_prop_initial ~  
                        te(Mass_per_loss, initial_N, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2, bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = Gamma(link = "inverse"),select = TRUE) 

Nper_Ni6 <- gam(data = det_loc, N_prop_initial ~  
                        te(Mass_per_loss, initial_N, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2, bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = Gamma(link = "log"),select = TRUE) 
...
Ni_mod_names <- list(Nper_Ni1,Nper_Ni2,Nper_Ni3,Nper_Ni4,Nper_Ni5,Nper_Ni6)
map(Ni_mod_names,AIC) #5 is best
map(Ni_mod_names, appraise) #not bad residuals!

summary(Nper_Ni5) #interaction looks significant

base_Nper_mod <- Nper_Ni5 #set best model to easier name to remember for later comparisons
...
#plot predictions
 ...
 Nper_Ni.pred<-predict_gam(type = "link", base_Nper_mod, length_out = 5000,  exclude_terms = c("s(series_index)", "s(series_index, Mass_per_loss)"), values = list(series_index = NULL,Remain_Mass_Category = NULL, initial_N = c(0.3,0.6,1.0,1.3,2)))%>%
  mutate(initial_N = as_factor(initial_N))
 
  base_Np_Ni.p<- ggplot()+
  geom_point(data = det_loc%>%filter(!is.na(N_per)), aes(Mass_per_loss, N_prop_initial), alpha = 0.3)+
    geom_smooth(data = Nper_Ni.pred, se = FALSE, aes(x = Mass_per_loss, y = 1/fit, color = initial_N))+
    geom_abline(slope = -0.01, intercept = 1)+
    #geom_ribbon(data = base_loc.pred, aes(x = Mass_per_loss, ymin = 1/(fit - 2*se.fit), ymax = 1/(fit + 2*se.fit)), alpha = 0.3)+
    NULL
 base_Np_Ni.p
...
```{r N per vs initial Lignin}
Nper_Li1 <- gam(data = det_loc, N_prop_initial ~  
                        te(Mass_per_loss, initial_Lignin, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category,m = 2, bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = gaussian(link = "identity"),select = TRUE) 

Nper_Li2 <- gam(data = det_loc, N_prop_initial ~
                te(Mass_per_loss, initial_Lignin, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2,bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = gaussian(link = "inverse"),select = TRUE) 
...
Nper_Li3 <- gam(data = det_loc, N_prop_initial ~  
                te(Mass_per_loss, initial_Lignin, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2,bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = gaussian(link = "log"),select = TRUE) 

Nper_Li4 <- gam(data = det_loc, N_prop_initial ~  
                te(Mass_per_loss, initial_Lignin, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2,bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = Gamma(link = "identity"),select = TRUE) 

Nper_Li5 <- gam(data = det_loc, N_prop_initial ~  
                te(Mass_per_loss, initial_Lignin, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2,bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = Gamma(link = "inverse"),select = TRUE) 

Nper_Li6 <- gam(data = det_loc, N_prop_initial ~  
                te(Mass_per_loss, initial_Lignin, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2,bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = Gamma(link = "log"),select = TRUE) 
...
Li_mod_names <- list(Nper_Li1, Nper_Li2,Nper_Li3,Nper_Li4,Nper_Li5,Nper_Li6)
map(Li_mod_names, ~AIC(.)) #5 best, then 2/6
map(Li_mod_names, ~appraise(.)) #residuals OK

summary(Nper_Li7)#Lignin probably not a significant driver of variation here



base_Li_mod <- Nper_Li7 #set best model to easier name to remember for later comparisons
...
base_loc_Li.pred<-predict_gam(type = "link", base_Li_mod, length_out = 5000,  exclude_terms = c("s(series_index)", "s(series_index, Mass_per_loss)"), values = list(series_index = NULL,Remain_Mass_Category = NULL, initial_Lignin = c(5,15,25,35)))%>%
 ...
 base_loc_Li.p<- ggplot()+
  geom_point(data = det_loc%>%filter(!is.na(initial_Lignin)), aes(Mass_per_loss, N_prop_initial), alpha = 0.3)+
    geom_smooth(data = base_loc_Li.pred, se = FALSE, aes(x = Mass_per_loss, y = 1/fit, color = initial_Lignin))+
    geom_abline(slope = -0.01, intercept = 1)+
    #geom_ribbon(data = base_loc.pred, aes(x = Mass_per_loss, ymin = 1/(fit - 2*se.fit), ymax = 1/(fit + 2*se.fit)), alpha = 0.3)+
    NULL
...
```{r N per vs Temperature}
Nper_T1 <- gam(data = det_loc, N_prop_initial ~  
                        te(Mass_per_loss, Temperature_C, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2,bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = gaussian(link = "identity"),select = TRUE) 

Nper_T2 <- gam(data = det_loc, N_prop_initial ~
                        te(Mass_per_loss, Temperature_C, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2,bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = gaussian(link = "inverse"),select = TRUE) 
...
Nper_T3 <- gam(data = det_loc, N_prop_initial ~  
                        te(Mass_per_loss, Temperature_C, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2,bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = gaussian(link = "log"),select = TRUE) 

Nper_T4 <- gam(data = det_loc, N_prop_initial ~  
                        te(Mass_per_loss, Temperature_C, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2,bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = Gamma(link = "identity"),select = TRUE) 

Nper_T5 <- gam(data = det_loc, N_prop_initial ~  
                        te(Mass_per_loss, Temperature_C, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2,bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = Gamma(link = "inverse"),select = TRUE) 

Nper_T6 <- gam(data = det_loc, N_prop_initial ~  
                        te(Mass_per_loss, Temperature_C, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2,bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = Gamma(link = "log"),select = TRUE) 

T_mod_names <- list(Nper_T1, Nper_T2,Nper_T3,Nper_T4,Nper_T5,Nper_T6)
map(T_mod_names, ~AIC(.)) #5 best then 4
map(T_mod_names, ~appraise(.)) #resids OK
summary(Nper_T5)#suggests Temp is not significant predictor



base_T_mod <- Nper_T5 #set best model to easier name to remember for later comparisons
...
base_loc_T.pred<-predict_gam(type = "link", base_T_mod, length_out = 5000,  exclude_terms = c("s(series_index)", "s(series_index, Mass_per_loss)"), values = list(series_index = NULL,Remain_Mass_Category = NULL, Temperature_C = c(5,10,15,20)))%>%
 ...
 base_loc_T.p<- ggplot()+
  geom_point(data = det_loc%>%filter(!is.na(Temperature_C)), aes(Mass_per_loss, N_prop_initial), alpha = 0.3)+
    geom_smooth(data = base_loc_T.pred, se = FALSE, aes(x = Mass_per_loss, y = 1/fit, color = Temperature_C))+
    geom_abline(slope = -0.01, intercept = 1)+
    #geom_ribbon(data = base_loc.pred, aes(x = Mass_per_loss, ymin = 1/(fit - 2*se.fit), ymax = 1/(fit + 2*se.fit)), alpha = 0.3)+
    NULL
...
```{r N mass vs TN}
Nper_TN1 <- gam(data = det_loc, N_prop_initial ~  
                        te(Mass_per_loss, TN_approx_avg, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category,m = 2, bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = gaussian(link = "identity"),select = TRUE) 

Nper_TN2 <- gam(data = det_loc, N_prop_initial ~
                te(Mass_per_loss, TN_approx_avg, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2,bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = gaussian(link = "inverse"),select = TRUE) 
...
Nper_TN3 <- gam(data = det_loc, N_prop_initial ~  
                te(Mass_per_loss, TN_approx_avg, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2,bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = gaussian(link = "log"),select = TRUE) 

Nper_TN4 <- gam(data = det_loc, N_prop_initial ~  
                te(Mass_per_loss, TN_approx_avg, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category,m = 2, bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = Gamma(link = "identity"),select = TRUE) 

Nper_TN5 <- gam(data = det_loc, N_prop_initial ~  
                te(Mass_per_loss, TN_approx_avg, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category,m = 2, bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = Gamma(link = "inverse"),select = TRUE) 

Nper_TN6 <- gam(data = det_loc, N_prop_initial ~  
                te(Mass_per_loss, TN_approx_avg, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2,bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = Gamma(link = "log"),select = TRUE) 

TN_mod_names <- list(Nper_TN1, Nper_TN2,Nper_TN3,Nper_TN4,Nper_TN5,Nper_TN6)
map(TN_mod_names, ~AIC(.)) #4,6 equivalent
map(TN_mod_names, ~appraise(.)) #4,6 still pretty equivalent

summary(Nper_TN4)#neither 4 nor 6 strong predictor



base_TN_mod <- Nper_TN4 #set best model to easier name to remember for later comparisons
...
base_loc_TN.pred<-predict_gam(type = "link", base_TN_mod, length_out = 5000,  exclude_terms = c("s(series_index)", "s(series_index, Mass_per_loss)"), values = list(series_index = NULL,Remain_Mass_Category = NULL, TN_approx_avg = c(250,750,1500)))%>%
 ...
 base_loc_TN.p<- ggplot()+
  geom_point(data = det_loc%>%filter(!is.na(TN_approx_avg)), aes(Mass_per_loss, N_prop_initial), alpha = 0.3)+
    geom_smooth(data = base_loc_TN.pred, se = FALSE, aes(x = Mass_per_loss, y = fit, color = TN_approx_avg))+
    geom_abline(slope = -0.01, intercept = 1)+
    #geom_ribbon(data = base_loc.pred, aes(x = Mass_per_loss, ymin = 1/(fit - 2*se.fit), ymax = 1/(fit + 2*se.fit)), alpha = 0.3)+
    NULL
 base_loc_TN.p
...
```{r N per vs TP}
Nper_TP1 <- gam(data = det_loc, N_prop_initial ~  
                        te(Mass_per_loss, TP_approx_avg, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category,m = 2, bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = gaussian(link = "identity"),select = TRUE) 

Nper_TP2 <- gam(data = det_loc, N_prop_initial ~
                te(Mass_per_loss, TP_approx_avg, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2,bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = gaussian(link = "inverse"),select = TRUE) 
...
Nper_TP3 <- gam(data = det_loc, N_prop_initial ~  
                te(Mass_per_loss, TP_approx_avg, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2,bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = gaussian(link = "log"),select = TRUE) 

Nper_TP4 <- gam(data = det_loc, N_prop_initial ~  
                te(Mass_per_loss, TP_approx_avg, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category,m = 2, bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = Gamma(link = "identity"),select = TRUE) 

Nper_TP5 <- gam(data = det_loc, N_prop_initial ~  
                te(Mass_per_loss, TP_approx_avg, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category,m = 2, bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = Gamma(link = "inverse"),select = TRUE) 

Nper_TP6 <- gam(data = det_loc, N_prop_initial ~  
                te(Mass_per_loss, TP_approx_avg, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2,bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = Gamma(link = "log"),select = TRUE) 

TP_mod_names <- list(Nper_TP1, Nper_TP2,Nper_TP3,Nper_TN4,Nper_TP5,Nper_TP6)
map(TP_mod_names, ~AIC(.)) #6 best, then 5
map(TP_mod_names, ~appraise(.)) #5,6 OK on resids

summary(Nper_TP6) #both 5 and 6 suggest TN significant interactor



base_TP_mod <- Nper_TP6 #set best model to easier name to remember for later comparisons
...
base_loc_TP.pred<-predict_gam(type = "link", base_TP_mod, length_out = 5000,  exclude_terms = c("s(series_index)", "s(series_index, Mass_per_loss)"), values = list(series_index = NULL,Remain_Mass_Category = NULL, TP_approx_avg = c(50,100,200,300,500)))%>%
 ...
 base_loc_TP.p<- ggplot()+
  geom_point(data = det_loc%>%filter(!is.na(TP_approx_avg)), aes(Mass_per_loss, N_prop_initial), alpha = 0.3)+
    geom_smooth(data = base_loc_TP.pred, se = FALSE, aes(x = Mass_per_loss, y = exp(fit), color = TP_approx_avg))+
    geom_abline(slope = -0.01, intercept = 1)+
    #geom_ribbon(data = base_loc.pred, aes(x = Mass_per_loss, ymin = 1/(fit - 2*se.fit), ymax = 1/(fit + 2*se.fit)), alpha = 0.3)+
    NULL
...
```{r Nper vs velocity}
Nper_vel1 <- gam(data = det_loc, N_prop_initial ~  
                        te(Mass_per_loss, Velocity_m.s, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category,m = 2, bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = gaussian(link = "identity"),select = TRUE) 

Nper_vel2 <- gam(data = det_loc, N_prop_initial ~
                te(Mass_per_loss, Velocity_m.s, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2,bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = gaussian(link = "inverse"),select = TRUE) 
...
Nper_vel3 <- gam(data = det_loc, N_prop_initial ~  
                te(Mass_per_loss, Velocity_m.s, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2,bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = gaussian(link = "log"),select = TRUE) 

Nper_vel4 <- gam(data = det_loc, N_prop_initial ~  
                te(Mass_per_loss, Velocity_m.s, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category,m = 2, bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = Gamma(link = "identity"),select = TRUE) 

Nper_vel5 <- gam(data = det_loc, N_prop_initial ~  
                te(Mass_per_loss, Velocity_m.s, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category,m = 2, bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = Gamma(link = "inverse"),select = TRUE) 

Nper_vel6 <- gam(data = det_loc, N_prop_initial ~  
                te(Mass_per_loss, Velocity_m.s, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2,bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re"), 
             method = "REML", family = Gamma(link = "log"),select = TRUE) 

vel_mod_names <- list(Nper_vel1, Nper_vel2,Nper_vel3,Nper_vel4,Nper_vel5,Nper_vel6)
map(TP_mod_names, ~AIC(.)) #6 best, then 5
map(TP_mod_names, ~appraise(.)) #5 maybe a little better than 6 on heteroscedasticity

summary(Nper_vel6)#5 suggests questionable significance, with 6 highly significant



base_vel_mod <- Nper_vel6 #set best model to easier name to remember for later comparisons
...
base_loc_vel.pred<-predict_gam(type = "link", base_vel_mod, length_out = 5000,  exclude_terms = c("s(series_index)", "s(series_index, Mass_per_loss)"), values = list(series_index = NULL,Remain_Mass_Category = NULL, Velocity_m.s = c(0.1,0.2,0.4)))%>%
 ...
 base_loc_vel.p<- ggplot()+
  geom_point(data = det_loc%>%filter(!is.na(Velocity_m.s)), aes(Mass_per_loss, N_prop_initial), alpha = 0.3)+
    geom_smooth(data = base_loc_vel.pred, se = FALSE, aes(x = Mass_per_loss, y = exp(fit), color = Velocity_m.s))+
    geom_abline(slope = -0.01, intercept = 1)+
    #geom_ribbon(data = base_loc.pred, aes(x = Mass_per_loss, ymin = 1/(fit - 2*se.fit), ymax = 1/(fit + 2*se.fit)), alpha = 0.3)+
    NULL
...
```{r}
CN_base <- gam(data = det_loc, CN_ratio ~
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2, bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re") + s(series_index, bs = "re"), 
             method = "REML", family = gaussian(link = "log"),select = TRUE) 
...
AIC(CN_base, CN_base2)
map(list(CN_base,CN_base2), appraise) #2 chosen due to less heteroscedastic resids, even if deltaAIC ~90

base_loc_CN.pred<-predict_gam(type = "link", CN_base2, length_out = 5000,  exclude_terms = c("s(series_index)", "s(series_index, Mass_per_loss)"), values = list(series_index = NULL, Remain_Mass_Category = NULL))



 base_loc_CN.p<- ggplot()+
  geom_point(data = det_loc%>%filter(!is.na(CN_ratio)), aes(Mass_per_loss, CN_ratio), alpha = 0.3)+
    geom_smooth(data = base_loc_CN.pred, se = FALSE, aes(x = Mass_per_loss, y = exp(fit)))+
    geom_abline(slope = -0.01, intercept = 1)+
    #geom_ribbon(data = base_loc.pred, aes(x = Mass_per_loss, ymin = 1/(fit - 2*se.fit), ymax = 1/(fit + 2*se.fit)), alpha = 0.3)+
    NULL
 base_loc_CN.p
...
Our real 'base' model for CN curves needs to include the initial_CN ratio, because the intercepts at the start of decomposition (0% mass loss) are, by definition, the CN ratio at the beginning. On our previous response variables, we collapsed these curves into proportional responses, but here we keep the curves so we can visualize the patterns of stoichiometric convergence. 
```{r CN by initial N}



CN_Ni1 <- gam(data = det_loc, CN_ratio ~  
...
CN_Ni2 <- gam(data = det_loc, CN_ratio ~  
                        te(Mass_per_loss, initial_CN, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2, bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re")+s(series_index, bs = "re"), 
             method = "REML", family = Gamma(link = "log"),select = TRUE) 
#exclusion of random intercept here, redundant with initial_CN, didn't change anything, estimated values adn pvals all the same



map(list(CN_Ni1, CN_Ni2, CN_Ni3),AIC) #1 is better
map(list(CN_Ni1, CN_Ni2, CN_Ni3), appraise) #heteroscedastic but not much to be done, as usual

summary(CN_Ni1) #interaction highly significant

base_Nper_mod <- CN_Ni1 #set best model to easier name to remember for later comparisons
...
#plot predictions
 ...
 CN_Ni.pred<-predict_gam(type = "link", CN_Ni2, length_out = 5000,  exclude_terms = c("s(series_index)", "s(series_index, Mass_per_loss)"), values = list(series_index = NULL,Remain_Mass_Category = NULL, initial_CN = c(20,40,60,80,110,145)))%>%
  mutate(initial_CN = as_factor(initial_CN))

 
  CN_Ni.p<- ggplot()+
  geom_point(data = det_loc%>%filter(!is.na(CN_ratio)), aes(Mass_per_loss, CN_ratio), alpha = 0.3)+
    geom_smooth(data = CN_Ni.pred, se = FALSE, aes(x = Mass_per_loss, y = exp(fit), color = initial_CN))+
    geom_abline(slope = -0.01, intercept = 1)+
    #geom_ribbon(data = base_loc.pred, aes(x = Mass_per_loss, ymin = 1/(fit - 2*se.fit), ymax = 1/(fit + 2*se.fit)), alpha = 0.3)+
    NULL
...
#not enough data to fit models, random effects had to be removed - CN also correlated with initial Lignin, so that is complicated, too
...
CN_Li3 <- gam(data = det_loc, CN_ratio ~  
                        te(Mass_per_loss, initial_CN, m = 2)+
                 ti(Mass_per_loss, Temperature_C, initial_CN, m =2)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2, bs = c("tp", "re"),full = TRUE), 
             method = "REML", family = Gamma(link = "log"),select = TRUE)
...
```
2 step process for checking modulation of CN curves here - first, check the best distributional assumption, then re-fit with maximum likelihood to compare fixed structures


```{r CN per vs Temperature}
...
CN_Ni_T1a <- gam(data = det_loc, CN_ratio ~  
                      
                        te(Mass_per_loss, initial_CN, Temperature_C, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2,bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re") + s(series_index, bs = "re"), 
             method = "REML", family = gaussian(link = "log"),select = TRUE) 

CN_Ni_T1b <- gam(data = det_loc, CN_ratio ~  
                      s(Mass_per_loss) + s(initial_CN)+
                        ti(Mass_per_loss, initial_CN, Temperature_C, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2,bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re") + s(series_index, bs = "re"), 
             method = "REML", family = gaussian(link = "log"),select = TRUE) 

CN_Ni_T1c <- gam(data = det_loc, CN_ratio ~  
                        ti(Mass_per_loss, initial_CN, Temperature_C, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2,bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re") + s(series_index, bs = "re"), 
             method = "REML", family = gaussian(link = "log"),select = TRUE) 

CN_Ni_T1d <- gam(data = det_loc, CN_ratio ~  
                   te(Mass_per_loss, initial_CN)+
                        ti(Mass_per_loss, initial_CN, Temperature_C, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2,bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re") + s(series_index, bs = "re"), 
             method = "REML", family = gaussian(link = "log"),select = TRUE) 

CN_Ni_T1e <- gam(data = det_loc, CN_ratio ~  
                   te(Mass_per_loss, initial_CN, m = 2)+
                   ti(Mass_per_loss, initial_CN, Temperature_C, m = 2)+ #tests whether interaction needed in addition to mass loss curves
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2,bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re") + s(series_index, bs = "re"), 
             method = "REML", family = gaussian(link = "log"),select = TRUE) 
...
map(list(CN_Ni_T1, CN_Ni_T2), ~AIC(.)) #1 substantially lower AIC better
map(list(CN_Ni_T1e), ~appraise(.)) #1 not bad on heteroscedasticity
summary(CN_Ni_T1e)#suggests Temp is not significant modulator



base_T_mod <- CN_Ni_T1e #set best model to easier name to remember for later comparisons


#plot predictions
CN_loc_T.pred<-predict_gam(type = "link", CN_Ni_T1, length_out = 5000,  exclude_terms = c("s(series_index)", "s(series_index, Mass_per_loss)"), values = list(series_index = NULL,Remain_Mass_Category = NULL, initial_CN = c(20,40,60,80,110,145),Temperature_C = c(5,10,15,20)))%>%
   mutate(initial_CN = as_factor(initial_CN),Temperature_C = as_factor(Temperature_C))
  
 CN_loc_T.p<- ggplot()+
  geom_point(data = det_loc%>%filter(!is.na(Temperature_C), !is.na(initial_CN)), aes(Mass_per_loss, CN_ratio), alpha = 0.3)+
    geom_smooth(data = CN_loc_T.pred2, se = FALSE, aes(x = Mass_per_loss, y = exp(fit), color = initial_CN, linetype = Temperature_C))+
    NULL
 CN_loc_T.p

 CN_loc_T.pred2<-predict_gam(type = "link", CN_Ni_T1e, length_out = 5000,  exclude_terms = c("s(series_index)", "s(series_index, Mass_per_loss)"), values = list(series_index = NULL,Remain_Mass_Category = NULL, initial_CN = c(20,40,60,80,110,145),Temperature_C = c(5,10,15,20)))%>%
   mutate(initial_CN = as_factor(initial_CN),Temperature_C = as_factor(Temperature_C))

 CN_loc_T.p2<- ggplot()+
 # geom_point(data = det_loc%>%filter(!is.na(Temperature_C), !is.na(initial_CN)), aes(Mass_per_loss, CN_ratio), alpha = 0.3)+
    geom_smooth(data = CN_loc_T.pred, se = FALSE, aes(x = Mass_per_loss, y = exp(fit), color = initial_CN, linetype = Temperature_C))+
    #geom_abline(slope = -0.01, intercept = 1)+
   facet_grid(Temperature_C~initial_CN, scales = "free")+
  geom_ribbon(data = CN_loc_T.pred, aes(x = Mass_per_loss, ymin = exp(fit - 2*se.fit), ymax = exp(fit + 2*se.fit), color = Temperature_C), alpha = 0.3)+
    NULL
 CN_loc_T.p2
...
```{r test cn v external}

CN_Ni_TN1 <- gam(data = det_loc, CN_ratio ~  
                        ti(initial_CN, TN_approx_avg, m = 2)+
                        te(Mass_per_loss, initial_CN, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2, bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re") +s(series_index, bs = "re"), 
             method = "REML", family = gaussian(link = "log"),select = TRUE) 

CN_Ni_TN1a <- gam(data = det_loc, CN_ratio ~  
                        ti(Mass_per_loss, initial_CN, TN_approx_avg, m = 2)+
                        te(Mass_per_loss, initial_CN, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2, bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re") +s(series_index, bs = "re"), 
             method = "REML", family = gaussian(link = "log"),select = TRUE) 

CN_Ni_TN1b <- gam(data = det_loc, CN_ratio ~  
                        ti(initial_CN,TN_approx_avg, m = 2)+    
                        ti(Mass_per_loss, initial_CN, TN_approx_avg, m = 2)+
                        te(Mass_per_loss, initial_CN, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2, bs = c("tp", "re"),full = TRUE) +
...
CN_Ni_TN1e <- gam(data = det_loc, CN_ratio ~  
                       # ti(initial_CN,TN_approx_avg, m = 2)+    
                       # ti(Mass_per_loss, initial_CN, TN_approx_avg, m = 2)+
                        te(Mass_per_loss, initial_CN,TN_approx_avg, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2, bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re") +s(series_index, bs = "re"), 
             method = "REML", family = gaussian(link = "log"),select = TRUE) 

#this took like 15 or 20 minutes...
CN_Ni_TN1full <- gam(data = det_loc, CN_ratio ~  
                        ti(initial_CN,TN_approx_avg, m = 2)+   
                        ti(Mass_per_loss,TN_approx_avg, m = 2)+  
                        ti(Mass_per_loss, initial_CN, TN_approx_avg, m = 2)+
                        te(Mass_per_loss, initial_CN, m = 2)+
...
CN_Ni_TN2 <- gam(data = det_loc, CN_ratio ~  
                        ti(initial_CN, TN_approx_avg, m = 2)+
                        te(Mass_per_loss, initial_CN, m = 2)+
                        t2(Mass_per_loss,Remain_Mass_Category, m = 2, bs = c("tp", "re"),full = TRUE) +
                        s(series_index, Mass_per_loss, bs = "re") +s(series_index, bs = "re"), 
...
map(list(CN_Ni_TN1,CN_Ni_TN2), AIC) #AIC a little better on 1
map(list(CN_Ni_TN1,CN_Ni_TN2), appraise) #both a  little heteroscedastic

summary(CN_Ni_TN1) #not significant

 CN_Ni_TN.pred<-predict_gam(type = "link", CN_Ni_TN1a, length_out = 5000,  exclude_terms = c("s(series_index)", "s(series_index, Mass_per_loss)"), values = list(series_index = NULL,Remain_Mass_Category = NULL, initial_CN = c(20,40,60,80,110,145), TN_approx_avg = c(300,700,1500)))%>%
  mutate(initial_CN = as_factor(initial_CN), TN_approx_avg = as_factor(TN_approx_avg))

 
CN_Ni.p<- ggplot()+
  geom_point(data = det_loc%>%filter(!is.na(CN_ratio),!is.na(TN_approx_avg)), aes(Mass_per_loss, CN_ratio), alpha = 0.3)+
    geom_smooth(data = CN_Ni_TN.pred, se = FALSE, aes(x = Mass_per_loss, y = exp(fit), color = initial_CN, linetype = TN_approx_avg))+
    geom_abline(slope = -0.01, intercept = 1)+
    #geom_ribbon(data = base_loc.pred, aes(x = Mass_per_loss, ymin = 1/(fit - 2*se.fit), ymax = 1/(fit + 2*se.fit)), alpha = 0.3)+
    NULL
 ...
 CN_Ni_TN.pred2<-predict_gam(type = "link", CN_Ni_TN1, length_out = 5000,  exclude_terms = c("s(series_index)", "s(series_index, Mass_per_loss)"), values = list(series_index = NULL,Remain_Mass_Category = NULL, initial_CN = c(20,40,60,80,110,145), TN_approx_avg = c(300,700,1500)))%>%
  mutate(initial_CN = as_factor(initial_CN), TN_approx_avg = as_factor(TN_approx_avg))

 
CN_Ni.p2<- ggplot()+
  geom_point(data = det_loc%>%filter(!is.na(CN_ratio),!is.na(TN_approx_avg)), aes(Mass_per_loss, CN_ratio), alpha = 0.3)+
    geom_smooth(data = CN_Ni_TN.pred2, se = FALSE, aes(x = Mass_per_loss, y = exp(fit), color = initial_CN, linetype = TN_approx_avg))+
    geom_abline(slope = -0.01, intercept = 1)+
    #geom_ribbon(data = base_loc.pred, aes(x = Mass_per_loss, ymin = 1/(fit - 2*se.fit), ymax = 1/(fit + 2*se.fit)), alpha = 0.3)+
    NULL
 ...
 CN_Ni.p2
  ...
  CN_Ni_TN.pred3<-predict_gam(type = "link", CN_Ni_TN1b, length_out = 5000,  exclude_terms = c("s(series_index)", "s(series_index, Mass_per_loss)"), values = list(series_index = NULL,Remain_Mass_Category = NULL, initial_CN = c(20,40,60,80,110,145), TN_approx_avg = c(300,700,1500)))%>%
  mutate(initial_CN = as_factor(initial_CN), TN_approx_avg = as_factor(TN_approx_avg))

 
CN_Ni.p3<- ggplot()+
  geom_point(data = det_loc%>%filter(!is.na(CN_ratio),!is.na(TN_approx_avg)), aes(Mass_per_loss, CN_ratio), alpha = 0.3)+
    geom_smooth(data = CN_Ni_TN.pred3, se = FALSE, aes(x = Mass_per_loss, y = exp(fit), color = initial_CN, linetype = TN_approx_avg))+
    geom_abline(slope = -0.01, intercept = 1)+
    #geom_ribbon(data = base_loc.pred, aes(x = Mass_per_loss, ymin = 1/(fit - 2*se.fit), ymax = 1/(fit + 2*se.fit)), alpha = 0.3)+
    NULL
 ...
 CN_Ni.p3
   ...
   CN_Ni_TN.pred4<-predict_gam(type = "link", CN_Ni_TN1e, length_out = 5000,  exclude_terms = c("s(series_index)", "s(series_index, Mass_per_loss)"), values = list(series_index = NULL,Remain_Mass_Category = NULL, initial_CN = c(20,40,60,80,110,145), TN_approx_avg = c(300,700,1500)))%>%
  mutate(initial_CN = as_factor(initial_CN), TN_approx_avg = as_factor(TN_approx_avg))

 
CN_Ni.p4<- ggplot()+
  geom_point(data = det_loc%>%filter(!is.na(CN_ratio),!is.na(TN_approx_avg)), aes(Mass_per_loss, CN_ratio), alpha = 0.3)+
    geom_smooth(data = CN_Ni_TN.pred4, se = FALSE, aes(x = Mass_per_loss, y = exp(fit), color = initial_CN, linetype = TN_approx_avg))+
    geom_abline(slope = -0.01, intercept = 1)+
    #geom_ribbon(data = base_loc.pred, aes(x = Mass_per_loss, ymin = 1/(fit - 2*se.fit), ymax = 1/(fit + 2*se.fit)), alpha = 0.3)+
    NULL
 ...
 CN_Ni.p4
    ...
    CN_Ni_TN.pred5<-predict_gam(type = "link", CN_Ni_TN1full, length_out = 5000,  exclude_terms = c("s(series_index)", "s(series_index, Mass_per_loss)"), values = list(series_index = NULL,Remain_Mass_Category = NULL, initial_CN = c(20,40,60,80,110,145), TN_approx_avg = c(300,700,1500)))%>%
  mutate(initial_CN = as_factor(initial_CN), TN_approx_avg = as_factor(TN_approx_avg))

 
CN_Ni.p5<- ggplot()+
  geom_point(data = det_loc%>%filter(!is.na(CN_ratio),!is.na(TN_approx_avg)), aes(Mass_per_loss, CN_ratio), alpha = 0.3)+
    geom_smooth(data = CN_Ni_TN.pred5, se = FALSE, aes(x = Mass_per_loss, y = exp(fit), color = initial_CN, linetype = TN_approx_avg))+
    geom_abline(slope = -0.01, intercept = 1)+
    #geom_ribbon(data = base_loc.pred, aes(x = Mass_per_loss, ymin = 1/(fit - 2*se.fit), ymax = 1/(fit + 2*se.fit)), alpha = 0.3)+
    NULL
...
map(list(CN_Ni.p,CN_Ni.p2,CN_Ni.p3), print)

#Each of these models includes global Mass loss X CN curves te(Mass_loss,initial_CN)
#1 - higher level crosses -  adds three way higher interaction - ti(Mass_per_loss, initial_CN, TN_approx_avg, m = 2)
#2 - not really any crossing - adds two way interaction - ti(initial_CN, TN_approx_avg, m = 2)
#3 - not really any crossing, wide divergence - adds two way interaction plus three way interaction -  ti(initial_CN,TN_approx_avg, m = 2)+ ti(Mass_per_loss, initial_CN, TN_approx_avg, m = 2) - this means CN curves are modulated by 

```

Summary and plots suggest TN doesn't modulate the CN convergence curves

The interaction for Temperature on CN is significant, but the plots really don't seem particularly illuminating
