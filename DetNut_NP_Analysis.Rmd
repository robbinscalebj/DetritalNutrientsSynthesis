...
#Base model
We potentially need several base models, though only one will be used for any given model comparison:
1) N mass ~ Mass loss+Lentic_Lotic (all coarse mesh), 
2) N mass ~ Mass loss (lotic coarse mesh only), 
3) N mass ~ Mass loss (lentic coarse mesh only), 
4) N mass ~ Mass loss+Lentic_Lotic (all fine mesh)
5) N mass ~ Mass loss (lotic fine mesh)
6) N mass ~ Mass loss (lentic fine mesh only)
...
N_base_loc1 <- gam(data = det_loc, N_mass_norm ~  
                        s(Mass_per_loss) + 
                        t2(Mass_per_loss,Remain_Mass_Category, bs = c("tp", "re")) +
...
#########Lentic coarse models
N_base_lec1 <- gam(data = det_lec, N_mass_norm ~  
                        s(Mass_per_loss) + 
                        t2(Mass_per_loss,Remain_Mass_Category, bs = c("tp", "re")) +
                        s(series_index, Mass_per_loss, bs = "re"), method = "REML", family = gaussian(link = "identity"),select = TRUE) 

N_base_lec2 <- gam(data = det_lec, N_mass_norm ~  
                        s(Mass_per_loss) + 
                        t2(Mass_per_loss,Remain_Mass_Category, bs = c("tp", "re")) +
                        s(series_index, Mass_per_loss, bs = "re"), method = "REML", family = gaussian(link = "inverse"),select = TRUE) 

N_base_lec3 <- gam(data = det_lec, N_mass_norm ~  
                        s(Mass_per_loss) + 
                        t2(Mass_per_loss,Remain_Mass_Category, bs = c("tp", "re")) +
                        s(series_index, Mass_per_loss, bs = "re"), method = "REML", family = gaussian(link = "log"),select = TRUE) 

N_base_lec4 <- gam(data = det_lec, N_mass_norm ~  
                        s(Mass_per_loss) + 
                        t2(Mass_per_loss,Remain_Mass_Category, bs = c("tp", "re")) +
                        s(series_index, Mass_per_loss, bs = "re"), method = "REML", family = Gamma(link = "identity"),select = TRUE) 

N_base_lec5 <- gam(data = det_lec, N_mass_norm ~  
                        s(Mass_per_loss) + 
                        t2(Mass_per_loss,Remain_Mass_Category, bs = c("tp", "re")) +
                        s(series_index, Mass_per_loss, bs = "re"), method = "REML", family = Gamma(link = "inverse"),select = TRUE) 

N_base_lec6 <- gam(data = det_lec, N_mass_norm ~  
                        s(Mass_per_loss) + 
                        t2(Mass_per_loss,Remain_Mass_Category, bs = c("tp", "re")) +
                        s(series_index, Mass_per_loss, bs = "re"), method = "REML", family = Gamma(link = "log"),select = TRUE) 
...
map(base_loc_names, ~AIC(.)) #2 is clear best
map(base_loc_names, ~appraise(.)) # 2 is OK for the bunch
map(base_loc_names, ~summary(.))

base_loc_mod <- gam(data = det_loc, N_mass_norm ~  
                        #s(Mass_per_loss) + 
                      te(initial_N,Mass_per_loss)+
                        t2(Mass_per_loss,Remain_Mass_Category, bs = c("tp", "re")),#+
                        #s(series_index, Mass_per_loss, bs = "re"), 
                      method = "REML", family = gaussian(link = "inverse"),select = TRUE)  #name for later comparisons



base_lec_names<-list(N_base_lec1,N_base_lec2,N_base_lec3,N_base_lec4,N_base_lec5,N_base_lec6)
map(base_lec_names, ~AIC(.)) #5 is best, 6 just behind
map(base_lec_names, ~appraise(.)) #I want to puke, but not much can be done about the heteroscedasticity
map(base_lec_names, ~summary(.))

base_lec_mod <- N_base_lec5

#plot predictions
base_loc.pred<-predict_gam(type = "link", base_loc_mod, length_out = 5000,  exclude_terms = c("s(series_index)", "s(series_index, Mass_per_loss)"), values = list(series_index = NULL,Remain_Mass_Category = NULL))
base_loc.pred2<-predict_gam(type = "link", base_loc_mod, length_out = 1000, values = list(series_index = NULL,Remain_Mass_Category = NULL, initial_N = c(0.6,1.2,1.8)))%>%mutate(initial_N = factor(initial_N))

 base_loc.p<- ggplot()+
  geom_point(data = det_loc, aes(Mass_per_loss, N_mass_norm), alpha = 0.3)+
    geom_smooth(data = base_loc.pred2,se = FALSE, aes(x = Mass_per_loss, y = 1/fit, color = initial_N))+
 ...
 base_lec.pred<-predict_gam(type = "link", N_base_lec4, length_out = 5000,  exclude_terms = c("s(series_index)", "s(series_index, Mass_per_loss)"), values = list(series_index = NULL))

 base_lec.p<- ggplot()+
  geom_point(data = det_lec, aes(Mass_per_loss, N_mass_norm), alpha = 0.3)+
    geom_smooth(data = base_lec.pred,se = FALSE, aes(x = Mass_per_loss, y = fit))+
    geom_abline(slope = -0.01, intercept = 1)+
   facet_grid(.~Remain_Mass_Category)+
    #geom_ribbon(data = base_lec.pred, aes(x = Mass_per_loss, ymin = 1/(fit - 2*se.fit), ymax = 1/(fit + 2*se.fit)), alpha = 0.3)+
    NULL
 base_lec.p
